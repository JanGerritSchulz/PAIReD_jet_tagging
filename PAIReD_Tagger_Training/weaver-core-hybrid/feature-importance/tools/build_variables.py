...
def _p4_from_pxpypze(px, py, pz, energy):
    ...
    return vector.Array(ak.zip({'px': px, 'py': py, 'pz': pz, 'energy': energy}))


def _p4_from_ptetaphie(pt, eta, phi, energy):
    import vector
    vector.register_awkward()
    return vector.Array(ak.zip({'pt': pt, 'eta': eta, 'phi': phi, 'energy': energy}))


def _p4_from_ptetaphim(pt, eta, phi, mass):
    import vector
    vector.register_awkward()
    return vector.Array(ak.zip({'pt': pt, 'eta': eta, 'phi': phi, 'mass': mass}))
